...
def deco_auth(isauth):
    def mod_isauth(request):
        if 'username' in request.session.keys() or request.user in User.objects.all():
...
def signup(request):
    if request.method == "POST":
        ...
        try:
            ...
            messages.error(request,"This email id or username is already in use")
            return redirect('signup')
    return render(request,'User/signup.html',{'cart_size':0})
...
def signin(request):
    ...
    return render(request,'User/signin.html',{'cart_size':0})
